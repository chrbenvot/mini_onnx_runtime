...
void test_relu()
{
    ...
    in_data[0] = -10.0f; in_data[1] = 5.0f; in_data[2] = -2.0f; in_data[3] = 0.0f;
    ...
    op.forward(inputs, outputs, node, workspace); // Added workspace
...
void test_conv_simple()
{
    ...
    w_ptr[0] = 1.0f; w_ptr[1] = 0.0f;
    w_ptr[2] = 0.0f; w_ptr[3] = 1.0f;

    onnx::NodeProto node;
    // pads = [0,0,0,0]
    auto *pads = node.add_attribute();
    pads->set_name("pads"); pads->add_ints(0); pads->add_ints(0); pads->add_ints(0); pads->add_ints(0);

    // strides = [1,1]
    auto *strides = node.add_attribute();
    strides->set_name("strides"); strides->add_ints(1); strides->add_ints(1);

    ConvOp op;
    Tensor output(DataType::FLOAT32, {1, 1, 1, 1}); 
...
void test_concat_upsample() {
    ...
    resize_op.forward(up_inputs, up_outputs, resize_node, workspace); // Added workspace
    ...
    concat_op.forward(cat_inputs, cat_outputs, concat_node, workspace); // Added workspace
